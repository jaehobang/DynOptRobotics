...
double run_sim_wo_save( double x[] )
{
  reinit_sim( &sim );
...
main( int argc, char **argv )
{
  ...
  //printf("the number of parameters in sim struct is: %d", sim.n_parameters);
  ...
  simplex(run_sim_wo_save, dvec_params, sim.n_parameters, .1, 1.0);
